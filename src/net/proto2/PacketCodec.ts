...
import BufferConsumer from "@net/protocol/BufferConsumer";
import FieldCodec from "@net/protocol/Field";
...
class PacketCodec<P extends KnownPacketFields> {
  ...
  public constructor(public id: number, name: string, public direction: Direction) {
    this.name = name.toLowerCase();
  }

  public addField<T extends string, F extends FieldCodec<any>, FT extends FieldCodecType<F>>(
  ...
  public async populate(consumer: BufferConsumer, current: KnownPacketFields) {
    ...
    for (const [key, fieldInfo] of this.packetFields.entries()) {
      ...
      if (fieldInfo.validator?.(val) ?? false) throw new STypeError("INVALID_PACKET");
      fields[key] = val;
