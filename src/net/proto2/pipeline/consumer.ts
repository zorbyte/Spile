function consumer(buffer: Buffer, maxLength = buffer.length) {
  let offset = 0;
  ...
  function consume(amount: number): Buffer;
  function consume(amount: number, changeLen: true): void;
  function consume(amount?: number, changeLen?: boolean): Buffer | number | void {
    ...
    if (endOffset > maxLength) throw new Error("Can not read outside bounds of consumer!");
